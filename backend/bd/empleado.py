...
def get_or_create_eps(eps:dict, mydb)->tuple|None:
    ...
    cursor = mydb.cursor()
    sql = ('INSERT INTO EPS (nombre) VALUES (%s)')
    val = (eps['nombre'],)
...
def get_or_create_cargo(cargo:dict, mydb)->tuple|None:
    ...
    sql = ('INSERT INTO CARGO (nombre) VALUES (%s)')
    val = (cargo['nombre'],)
    cursor.execute(sql, val)
    mydb.commit()
...
def get_or_create_pension(pension:dict, mydb)->tuple|None:
    ...
    sql = ('INSERT INTO PENSION (nombre) VALUES (%s)')
    val = (pension['nombre'],)
    cursor.execute(sql, val)
    mydb.commit()
...
def get_or_create_dependencia(dep:dict, mydb)->tuple:
    ...
    sql = ('INSERT INTO DEPENDENCIA (nombre) VALUES (%s)')
    val = (dep['nombre'],)
    cursor.execute(sql, val)
    mydb.commit()
...
def get_or_create_empleado(emp:dict, mydb)-> tuple|None:
    ...
    sql = ('INSERT INTO EMPLEADO (codigo, sueldo, arl, '+
...
def insertar_novedades(novedad:List[Dict], mydb):
    ...
    sql = ('INSERT IGNORE INTO NOVEDAD (fecha_inicio, fecha_fin, dias_trabajados, '+
...
def get_empleados(quantity, page, mydb):

    cursor = mydb.cursor()

    cursor.execute(f'SELECT * FROM EMPLEADO LIMIT {quantity} OFFSET {quantity*page - quantity} ')
